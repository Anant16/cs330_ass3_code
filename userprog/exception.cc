...
ExceptionHandler(ExceptionType which)
{
	...
	DEBUG('a', "Shutdown, initiated by user program.\n");
       ...
       TranslationEntry *oldKernelPageTable = currentThread->space->GetPageTable();
       for (i = 0; i < numVirtualPages; i++) {
	        KernelPageTable[i].virtualPage = oldKernelPageTable[i].virtualPage;
	        KernelPageTable[i].physicalPage = oldKernelPageTable[i].physicalPage;
